var originAirport = 'MCO',
    ...
    apiRoot = 'http://njosnavel.in:7919/api/city-pairs-2014',
    store = {};
...
function initCPP() {
    ...
    var req = apiRoot + '/values/ORIGIN_CITY';
    console.log(req)
    ...
    $.ajax({
        ...
        origins = json.values;
        //temp fix...should be remedied in okapi
        origins.splice(219,1)

        console.log(origins)

        /*for(i in origins){
            var req2 = apiRoot + '?ORIGIN_CITY=' + origins[i];
             $.ajax({
                url: req2,
                dataType: "jsonp",
                timeout: 2500
            }).success(function(json2) {
                var state = json2.results[0].state;
                origins[i] = origins[i] + ' ' + state;
            });
        }*/

        console.log(origins)

        $('#cpp-origin').autocomplete({
            lookup: origins,
            lookupLimit: 5,
            appendTo: $('#cpp-origin-autocomplete')
        }).on('keyup',function(e){
            //IGNORE ARROW KEYS
            if (e.keyCode !== 37 && e.keyCode !== 38 && e.keyCode !== 39 && e.keyCode !== 40) {
                ...
                if(origins.indexOf(value) > -1){
                    if($('.autocomplete-suggestions').css('display') == 'none'){
                        findCPPdestinations(value);
                    }
                    else{
                        findCPPdestinations(value,false);
                    }
                }
            }
        });
...
function findCPPrate(origin, destination) {
    var html,
        ...
        req = apiRoot + '?ORIGIN_CITY=' + origin + '&DESTINATION_CITY_NAME=' + destination;
    console.log(req)
    //FIND THE RATE
    $.ajax({
        url: req,
        ...
        //TEMPLATE
        var effective = result['EFFECTIVE_DATE'].replace(' 0:00', ''),
            expiration = result['EXPIRATION_DATE'].replace(' 0:00', '');
        html = template
            .replace(/{{origin}}/g, origin+' ('+result['ORIGIN_AIRPORT']+')')
            .replace(/{{destination}}/g, destination+' ('+result['DESTINATION_AIRPORT']+')')
            .replace(/{{airline}}/g, result['AIRLINE_ABBREVIATION'])
            .replace(/{{service}}/g, result['AWARDED_SERVICE'])
            .replace(/{{yca}}/g, result['YCA_FARE'])
            .replace(/{{ca}}/g, result['_CA_FARE'])
            .replace(/{{cb}}/g, result['BUSINESS_FARE'])
            .replace(/{{effective}}/g, effective)
            .replace(/{{expiration}}/g, expiration);
        //APPEND
        $('#cpp-rate').html('').append(html).addClass('fadeInDown animated');
        setTimeout(function() {
            $('#cpp-rate').removeClass('fadeInDown animated');
        }, 1000);
    });
};

function findCPPdestinations(originCity, animate) {
    var destinations = [],
        req = apiRoot + '?ORIGIN_CITY=' + originCity;
    console.log(req)
    //FIND DESTINATIONS
    $.ajax({
        url: req,
        ...
        for (var i = 0; i < results.length; i++) {
            console.log(results[i]['DESTINATION_CITY_NAME'])
            if (destinations.indexOf(results[i]['DESTINATION_CITY_NAME']) === -1) {
                destinations.push(results[i]['DESTINATION_CITY_NAME'])
            }
        }
        ...
        if (animate !== false) {
            ...
            setTimeout(function() {
                $('#cpp-destination').removeClass('animated pulse');
            }, 1000)
        }
